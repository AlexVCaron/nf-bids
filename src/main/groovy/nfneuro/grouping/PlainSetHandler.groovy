...
class PlainSetHandler extends BaseSetHandler {
    ...
    private BidsChannelData processPlainSetFile(
        ...
        if (!suffix) {
            BidsLogger.logProgress("nf-bids-plain-set", "Skipping file without suffix: ${file.path}")
            return null
        }
        ...
        if (!suffixConfig) {
            BidsLogger.logProgress("nf-bids-plain-set", "No configuration for suffix: ${suffix} - FILTERED")
            return null
        }

        def plainSetConfig = getSetConfig(suffixConfig)
        if (plainSetConfig == null) {
            BidsLogger.logProgress("nf-bids-plain-set", "No plain_set configuration for suffix: ${suffix} - FILTERED")
            return null
        }

        BidsLogger.logProgress("nf-bids-plain-set", "Found plain_set config for ${suffix}")
        ...
        if (plainSetConfig.filter) {
            if (!BidsEntityUtils.entitiesMatch(file.entities, plainSetConfig.filter as List)) {
                BidsLogger.logProgress("nf-bids-plain-set", "File filtered by entity pattern: ${file.path}")
                return null
            }
        }
        ...
        if (plainSetConfig.exclude_entities) {
            for (String entityName : plainSetConfig.exclude_entities as List<String>) {
                ...
                if (entityValue && entityValue != "NA") {
                    BidsLogger.logProgress("nf-bids-plain-set", "File excluded by entity ${entityName}: ${file.path}")
                    return null
                }
            }
        }
        ...
        if (requiredEntities && !BidsEntityUtils.hasRequiredEntities(file, requiredEntities)) {
            BidsLogger.logProgress("nf-bids-plain-set", "File missing required entities: ${file.path}")
            return null
        }

        // Build grouping key from loop-over entities
        def groupingKey = BidsEntityUtils.groupingKey(file, loopOverEntities)

        // Create channel data structure
        def channelData = new BidsChannelData()

        // Build nested data map grouping files by extension type
        def nestedDataMap = nestedDataMap(datasetRoot, file, allFiles)
        ...
        BidsLogger.logProgress("nf-bids-plain-set", "Plain set emitted: ${file.filename} with key ${groupingKey}")
