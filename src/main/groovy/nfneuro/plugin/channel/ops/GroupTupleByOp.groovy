...
class GroupTupleByOp {
    ...
    // Grouping state
    private final Map<Object, List<Object>> groups = new HashMap<>()
    ...
    DataflowWriteChannel apply() {
        ...
        final next = { item -> processItem(item) }
        final done = { completeGrouping() }
    ...
    private synchronized void emitGroup(Object key) {
        ...
        if (sort) {
            if (sort instanceof Boolean && sort) {
                ...
                log.warn("groupTupleBy: Invalid sort option type: ${sort.class.name}, skipping sort")
            }
        }
