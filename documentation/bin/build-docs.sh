...
prepare_publish_site() {
  local site_root="${DOC_DIR}/build/site"
  local component_root="${site_root}/nf-bids"
  local latest_version=""

  [[ -d "${component_root}" ]] || fail "Missing component output directory: ${component_root}"

  rm -rf "${PUBLISH_DIR}"
  mkdir -p "${PUBLISH_DIR}"

  # Shared UI assets generated by Antora.
  if [[ -d "${site_root}/_" ]]; then
    rsync -a "${site_root}/_/" "${PUBLISH_DIR}/_/"
  fi

  # Optional top-level files Antora may generate.
  for f in 404.html sitemap.xml robots.txt; do
    [[ -f "${site_root}/${f}" ]] && cp "${site_root}/${f}" "${PUBLISH_DIR}/${f}"
  done

  # Flatten component output so GH Pages project sites do not duplicate /nf-bids/nf-bids/...
  rsync -a "${component_root}/" "${PUBLISH_DIR}/"

  latest_version="$(find "${component_root}" -mindepth 1 -maxdepth 1 -type d -printf '%f\n' | sort -V | tail -n1 || true)"
  [[ -n "${latest_version}" ]] || fail "Could not determine latest version under ${PUBLISH_DIR}"

  cat > "${PUBLISH_DIR}/index.html" <<EOF
<!DOCTYPE html>
<meta charset="utf-8">
<script>location="${latest_version}/index.html"</script>
<meta http-equiv="refresh" content="0; url=${latest_version}/index.html">
<meta name="robots" content="noindex">
<title>Redirect Notice</title>
<h1>Redirect Notice</h1>
<p>The page you requested has been relocated to <a href="${latest_version}/index.html">${latest_version}/index.html</a>.</p>
EOF

  log "Prepared publish-ready site at ${PUBLISH_DIR}"
}
