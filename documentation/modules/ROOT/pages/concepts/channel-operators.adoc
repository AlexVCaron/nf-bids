...
Emits fused joined items for every matching pair (the join key is not emitted).
...
// Unmatched items emit the available side as-is
----
...
Emits fused combined items (without keys) — the full _cartesian product_ of matching
