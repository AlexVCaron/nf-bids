...
[source,groovy]
----
plugins { id 'nf-bids@0.1.0-beta.10' }
----
...
NOTE: Both `join` and `joinBy` emit `[key, leftItem, rightItem]`.  The closure-based
version includes the key — do not destructure as `{ l, r -> }`.
...
// Closure-based: same semantics, semantic key
subjects.combineBy(sessions, { it.subject })
// Both emit [key, leftItem, rightItem]
----
...
scans.combineBy(protocols, { it.modality })
     .filter { key, scan, proto -> scan.modality == proto.modality }
----

== Common pitfalls

=== joinBy output includes the key

[source,groovy]
----
// ❌ WRONG — joinBy emits [key, left, right], not [left, right]
left.joinBy(right) { it.id }.view { l, r -> println l }
...
* xref:concepts/channel-operators.adoc[Closure-Based Channel Operators — full reference]
