...
| `JoinByOp`
| Inner-joins two channels by closure-extracted keys; emits `[key, leftItem, rightItem]` tuples.
...
Channel.fromBIDS(
    ...
    Map    options    = [:]   // libbids_sh, flatten_output, bids_validation
)
----
...
emits unmatched items with `null` partner.  Emits `[key, leftItem, rightItem]`.
...
matched key group.  Unmatched keys are silently dropped (inner semantics).
Emits `[key, leftItem, rightItem]`.
|===
